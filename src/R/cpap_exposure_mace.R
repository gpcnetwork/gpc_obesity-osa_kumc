...
for(mace_endpt in mace_endpts){
  ...
  path_to_file<-file.path(
    path_to_data_folder,
    paste0("cpap_exposure_",mace_endpt,"_adjset_bts",boots,".rda"
    ))
  ...
  for(boot_i in boots_iter){
    ...
    if(!file.exists(path_to_file)){
      ...
      for(i in seq_len(nrow(strata))){
        ...
        fit_frm<-formula(paste0("Surv(",mace_endpt,"_time, ",mace_endpt,"_status) ~ ",
      ...
      write.csv(result,file=path_to_file,row.names = F)
    }
    
    #==== IPW-adjusted, with interactions ==========
    #-- built IPW-adjusted model within each strata
    path_to_file<-file.path(path_to_dir,"coxph_strata_intx.csv")
    if(!file.exists(path_to_file)){
      result<-c()
      for(i in seq_len(nrow(strata))){
        #- model training
