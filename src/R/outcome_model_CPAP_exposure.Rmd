...
fitcox<-readRDS(file.path(path_to_datadir,"surv","boot1","coxph_iptw_main.rda"))
...
fitcox<-readRDS(file.path(path_to_datadir,"results","mace","boot1","coxph_iptw_main.rda"))
...
# kmplot
# save figure
ggsave(file.path(path_to_dir,"res","cpap_expo_on_surv_km.pdf"),
       plot = kmplot,
       dpi = 300,
...
fplt1<-forestplot.HR(
  df = dat %>% filter(endpt %in% c("surv","mace") & model =="main" & grepl("^(0[1-9])+",stratum_var_lbl)),  # long table
...
# save figure
ggsave(file.path(path_to_dir,"res","cpap_expo_on_surv_mace_p1.pdf"),
       plot = fplt1,
       dpi = 300,
...
# save figure
ggsave(file.path(path_to_dir,"res","cpap_expo_on_surv_mace_p2.pdf"),
       plot = fplt2,
       dpi = 300,
       width = 12, height = 12, units = "in")
```
...
fplt
# save figure
ggsave(file.path(path_to_dir,"res","cpap_expo_on_mace_events.pdf"),
       plot = fplt,
       dpi = 300,
