...
set cdm_db_schema = 'GROUSE_DB_BLUE.CMS_PCORNET_CDM';
set diagnosis = $cdm_db_schema || '.DIAGNOSIS';
set procedures = $cdm_db_schema || '.PROCEDURES';
set enrollment = $cdm_db_schema || '.ENROLLMENT';
set demographic = $cdm_db_schema || '.DEMOGRAPHIC';
set death = $cdm_db_schema || '.DEATH';
set address = $cdm_db_schema || '.ADDRESS_HISTORY';
...
create or replace table PAT_ENR_65TO79 as
with pat65up_cte as (
      select distinct d.PATID, d. BIRTH_DATE
      from identifier($demographic) d
      join identifier($enrollment) e
      on d.PATID = e.PATID
      where floor(datediff('day',d.BIRTH_DATE,e.ENR_START_DATE)/365.25) between 65 and 79
)
select e.*, p.BIRTH_DATE
from identifier($enrollment) e
join pat65up_cte p on p.PATID = e.PATID
;
select count(distinct patid) from PAT_ENR_65TO79;
-- 10,848,485

/*cohort initialization: at least 1 OSA*/
create or replace table PAT_OSA_1DX as
...
;
select count(distinct PATID) from PAT_OSA_1DX;
-- 1,795,214
